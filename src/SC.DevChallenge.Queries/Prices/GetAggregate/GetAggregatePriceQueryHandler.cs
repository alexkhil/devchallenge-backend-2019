...
namespace SC.DevChallenge.Queries.Prices.GetAggregate
{
    public class GetAggregatePriceQueryHandler : QueryHandlerBase<GetAggregatePriceQuery, List<AggregatePriceViewModel>>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var avgPriceByTimeslot =
                await this.priceRepository.GetAveragePricesAsync(request.Portfolio, startTimeSlot, endTimeSlot);
            ...
            var groupCounts = Enumerable.Range(1, request.ResultPoints)
                .Select((x, i) => i + 1 <= remainder ? elementsInGroup + 1 : elementsInGroup).ToList();
            ...
            foreach (var num in groupCounts)
            {
                ...
                result.Add(new AggregatePriceViewModel
                {
                    ...
                    Price = Math.Round(
                        avgPriceByTimeslot.Where(x => x.Key >= start && x.Key <= end).Average(x => x.Value), 2)
                });
                start = end + 1;
            }
